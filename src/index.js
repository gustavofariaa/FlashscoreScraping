...
(async () => {
  ...
  try {
    ...
    const { fileName, season, fileType } = await promptUserOptions(browser, cliOptions);
    ...
    console.info(`📁 File saved to: ${chalk.cyan(`${OUTPUT_PATH}/${fileName}${fileType.extension}`)}\n`);
  } catch (error) {
    stop();
